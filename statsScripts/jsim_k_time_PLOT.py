...
for pairs in pairsFile.readlines():
    ...
    dt = pd.read_csv(path + '\\data\\jsim_time_' + str(pairs[0]) + '_' + str(pairs[1]) + '.csv')
    # Plot the data
    x = dt.iloc[:, 0]
    y = dt.iloc[:, 1]
    ax.plot(x, y, label='time for docs ' + str(pairs[0]) + ' and ' + str(pairs[1]), alpha=1.0)
...
# Set titles
ax.set(title="Average (of 5) Time to compute Jaccard Similarity", xlabel="shingle size (num chars)", ylabel="time (ms)")

# Show the plot
# plt.show()

# Save the plot
plt.savefig(path + '\\plots\\jsim_k_time.png', bbox_inches='tight')
