...
dt = pd.read_csv(path + '\\data\\sig_error_modular_vs_jenkinsFunc.csv', sep='\s*,\s*')
...
# Set titles
ax.set(title="(error from Modular Function) - (error from Jenkins Function)", xlabel="shingles size (k)", ylabel="mean error difference (%)")

# Show the plot
# plt.show()

# Save the plot
plt.savefig(path + '\\plots\\sig_error_modular_vs_jenkinsFunc.png', bbox_inches='tight')
