...
If we succeed in shingling the documents using \textit{k}-singles, we only will have to compare all pairs of documents using the Jaccard Index and say if there is similarity between them or not. By doing this, we have two problems: time and space complexity.
...
In typical aplications the matrix is sparse, which means that there are more 0s than 1s. \underline{We can demonstrate this by calculating the probability of an element of the set to belong to a document D.}

If we take \textit{k} shingles, then the document have relatively few of the possible shingles.
Another way to think about this is with the toys in Christmas Day. Usually, kids would like to have an specific toy, which is very popular at that moment. Then, lots of toys would not be buyed for (a kid/anyone)?.
