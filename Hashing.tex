...
In this (practica) we are going to create differents algorithms to compute the difference between various documents in order to be able to detect if there may have been a possibility of copy any pair of the documents.
To do it, we have started with a very basic algorithm that returns the Jaccard Similarity with two documents and we have finished obtaining a Locality-sensitive hashing algorithm that giving the paths of several documents will return which documents may have been copied. (All the algorithms are explained in the document).
...
\begin{center}
	...
	\textit{Figure \ref{fig:typesValues}: Representation of the matrix with all the possible permutations.}
\end{center}
...
Tots ESTAN FETS AMB PYTHON! PELS EXPERIMENTS HEM TRIAT ALEATORIAMENT PARELLES DE DOCUMENTS DE 50 PARAULES PERMUTADES ALEATORIAMENT! CODI? <- APPENDIX? CADA PLOT AMB 5 MOSTRES
\subsection{Jaccard Similarity}
...
\begin{center}
	...
	\textit{Figure \ref{fig:jsim_k_sim.png}: Jaccard Similarity varying the \textit{k} value.}
\end{center}
...
\begin{figure}[!htb]
   \begin{minipage}{0.48\textwidth}
     ...
     \caption{Time to compute the Jaccard Similarity when varying the \textit{k} value.}\label{Fig:jsim_k_time.png}
   \end{minipage}\hfill
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=.9\linewidth]{jsim_k_meanTime.png}
     \caption{Average time to compute the Jaccard Similarity of the Figure \ref{Fig:jsim_k_time.png}.}\label{Fig:jsim_k_meanTime.png}
   \end{minipage}
\end{figure}
...
\begin{center}
	...
	\textit{Figure XXXXXXXXXXXXX: Time to compute the Jaccard Similarity when varying the \textit{k} value.}
\end{center}
...
\begin{center}
	...
	\textit{Figure XXXXXXXXXXXXXXXXX: Jaccard Similarity aproximation varying the shingle sizes.}
\end{center}

 The similarity between the documents is strictly related with the number of hashing functions that we use. When the number of hashing functions is a small value it is very different than the Jaccard Similarity, because the signature matrix has few rows and then, there are few chances that one row has the same hash values on two different columns. But when the number of hash functions increases, the functions are more alike to the Jaccard Similarity (Figure YYYYYYYYYYYYYYYYYYYYYYYY 7.1.1.1.1.1.1.1).
...
\begin{figure}[!htb]
   \begin{minipage}{0.48\textwidth}
     ...
     \caption{Jaccard Similarity approximation varying the number of hash function and the size of the shingles.}\label{Fig:signature_nHash_k_meanSim3D.png}
   \end{minipage}\hfill
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=1.08\linewidth]{signature_nHash_k_meanTime.png}
     \caption{Average time to compute the Jaccard Similarity approximation varying the shingle sizes.}\label{Fig:signature_nHash_k_meanTime.png}
   \end{minipage}
\end{figure}

We can see in Figure \ref{Fig:signature_nHash_k_meanSim3D.png} that the plot form begins to stay stable from number of hash functions equal to 200 more or less. Furthermore, in Figure \ref{Fig:signature_nHash_k_meanTime.png} the time cost starts to increase a lot from number of hash function equal to 250.
...
It is important to remark that in Figure \ref{Fig:signature_nHash_k_meanTime.png} the time stabilize near the \textit{k} equal to the average word length. FALTAAAAAAAAAAAAAAAAAAAAA DIR EL PERQUÈ CREIES QUE PASSA AIXÒ??????????? SI ÉS AIXÍ NO ESTIC MOLT SEGUR DE LA RESPOSTA XD.

\hfill

In Figures \ref{Fig:signature_jsim_meanError.png}, \ref{Fig:signature_jsim_meanError_from25.png} and \ref{Fig:signature_jsim_meanError_from50.png} there is represented how much is the error value obtained using the Jaccard Similarity and the Signature Matrix Approximation. Approximately we can arrive to the same conclusions as the ones that are before. The more number of hash functions, the less the error is.

\begin{figure}[!htb]
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=.9\linewidth]{signature_jsim_meanError.png}
     \caption{Error between the Jaccard Similarity and the Jaccard Similarity approximated in the signature matrix.}\label{Fig:signature_jsim_meanError.png}
   \end{minipage}\hfill
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=0.9\linewidth]{signature_jsim_meanError_from25.png}
     \caption{The same than Figure \ref{Fig:signature_jsim_meanError.png} but starting from nHashes = 25.}\label{Fig:signature_jsim_meanError_from25.png}
   \end{minipage}
\end{figure}

\begin{figure}
\centering
     \includegraphics[width=0.48\linewidth]{signature_jsim_meanError_from50.png}
     \caption{The same than Figure \ref{Fig:signature_jsim_meanError.png} but starting from nHashes = 50.}\label{Fig:signature_jsim_meanError_from50.png}
\end{figure}
...
\begin{center}
	...
	\textit{Figure \ref{fig:signature_nHash_k_meanSim_jenkinsFunc.png}: Jaccard Similarity aproximation varying the \textit{k} value.}
\end{center}
...
\begin{figure}[!htb]
   \begin{minipage}{0.48\textwidth}
     ...
     \caption{Time to compute the Jaccard Similarity aproximation when varying the \textit{k} value.}\label{Fig:signature_jsim_meanError_jenkinsFunc.png}
   \end{minipage}\hfill
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=.9\linewidth]{signature_nHash_k_meanTime_jenkinsFunc.png}
     \caption{Average time to compute the Jaccard Similarity aproximation varying the shingles sizes.}\label{Fig:jsim_k_meanTime.png}
   \end{minipage}
\end{figure}
...
\begin{figure}[!htb]
   ...
   \begin{minipage}{0.48\textwidth}
     ...
     \caption{The same than Figure \ref{Fig:signature_jsim_meanError.png} but starting from nHashes = 25.}\label{Fig:signature_jsim_meanError_from25_jenkinsFunc.png}
   \end{minipage}
\end{figure}

\begin{figure}
\centering
     \includegraphics[width=0.48\linewidth]{signature_jsim_meanError_from25_jenkinsFunc.png}
     \caption{The same than Figure \ref{Fig:signature_jsim_meanError.png} but starting from nHashes = 50.}\label{Fig:signature_jsim_meanError_from25_jenkinsFunc.png}
\end{figure}
...
\begin{figure}[!htb]
   \begin{minipage}{0.48\textwidth}
     ...
     \caption{Diference between the error using the modular hashing and the one using the jenkins hashing.}\label{Fig:sig_error_modular_vs_jenkinsFunc.png}
   \end{minipage}\hfill
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=0.9\linewidth]{sig_error_modular_vs_jenkinsFunc_from25.png}
     \caption{The same than Figure \ref{Fig:sig_error_modular_vs_jenkinsFunc.png} but starting from nHashes = 25.}\label{Fig:sig_error_modular_vs_jenkinsFunc_from25.png}
   \end{minipage}
\end{figure}

\begin{figure}
\centering
     \includegraphics[width=0.48\linewidth]{sig_error_modular_vs_jenkinsFunc_from50.png}
     \caption{The same than Figure \ref{Fig:sig_error_modular_vs_jenkinsFunc.png} but starting from nHashes = 50.}\label{Fig:sig_error_modular_vs_jenkinsFunc_from50.png}
\end{figure}

\break

We can see that the error using the modular hashing in Figures \ref{Fig:sig_error_modular_vs_jenkinsFunc.png}, \ref{Fig:sig_error_modular_vs_jenkinsFunc_from25.png} and \ref{Fig:sig_error_modular_vs_jenkinsFunc_from50.png} is greater most of the time, specially at the begining. Thus, the Jaccard Similarity approximation using the jenkins hashing is better than the one using the modular hashing.

In the Figure \ref{Fig:sig_time_modular_vs_jenkinsFunc.png} we have represented the time cost using modular and jenkins hashing. If we are in the positive Y, the modular functions spends more time on the algorithm. But we see that it is the contrary: the jenkins hashing is the one that spends more time even though it has less error than the modular hashing.
 
\begin{figure}
\centering
     \includegraphics[width=0.48\linewidth]{sig_time_modular_vs_jenkinsFunc.png}
     \caption{Difference of the time between the modular function and the jenkins function.}\label{Fig:sig_time_modular_vs_jenkinsFunc.png}
\end{figure}
